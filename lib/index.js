...
class Sense {
  constructor({ number, status, meaning, verbal_illustration, synonyms, senses }) {
    if (number) {
      this.number = number
    }
    if (senses) {
...
function build_hierarchy(senses) {
  let stack = []
...
function dt_walker(dtNode, sense) {
      ...
      .then(doc => {
        ...
        return Array.prototype.slice.call(entries).map(entry => {
          return {
            ...
            definition: def_walker(entry.getElementsByTagName('def')[0])
          }
        })
      })
  }
