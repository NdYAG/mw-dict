...
function get_definition(texts = [], numbers = [], statuses = []) {
  ...
  return texts.map((def, i) => {
    ...
    if (isPlainObject(def)) {
      ...
      // references
      if (!synonyms) {
        synonyms = fw // fw: abbreviation word expansion
      }
      return new Sense({
        sense_number,
        status,
...
class Sense {
  constructor({ sense_number, status, meaning, verbal_illustration, synonyms }) {
  ...
  _normalizeSn(sense_number) {
    if (sense_number) {
      ...
      if (isPlainObject(sense_number)) {
        let { _, snp } = sense_number
        result = `${_ || ''}${snp}`
      }
      return `**${result}**`
    }
  ...
  format() {
    ...
    this._normalizeSx(this.synonyms),
